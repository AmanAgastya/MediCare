...
const API_BASE = process.env.REACT_APP_API_URL || 'http://localhost:5000'; // ✅ FIXED
...
const DoctorLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/auth/hospital/doctor-login`, { // ✅ FIXED
        method: 'POST',
      ...
      if (!contentType || !contentType.includes('application/json')) {
        setError(res.status === 502 || res.status === 503
